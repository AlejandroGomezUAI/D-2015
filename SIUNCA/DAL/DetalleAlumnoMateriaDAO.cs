    ...
    public class DetalleAlumnoMateriaDAO
    {
        public List<Alumno_MateriaCC> TraerTodo(Alumno unAlumno)
        {
            ...
            try
            {
                ...
                resultado = unaConexion.EjecutarTupla<Alumno_MateriaCC>("SELECT MateriaConCorrelativas.Nombre FROM MateriaConCorrelativas INNER JOIN Alumno_MateriaCC on Alumno_MateriaCC.IdMateriaCC = MateriaConCorrelativas.IdMateriaCC INNER JOIN Alumno on Alumno.LegajoAlumno = Alumno_MateriaCC.LegajoAlumno WHERE Alumno_MateriaCC.Estado = 'Aprobado' AND Alumno.LegajoAlumno = (@LegajoAlumno)", listaParametrosCD);
